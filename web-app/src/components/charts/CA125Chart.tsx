      ...
      <CardHeader className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 p-6">
        ...
        <div className="flex gap-2">
          <Button
            ...
            className="gap-1.5 text-xs font-bold text-slate-500 bg-white border-slate-200 hover:bg-slate-50 hover:text-orange-600"
            title="Descargar CSV"
          >
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center overflow-x-auto pb-1 sm:pb-0">
          ...
          <Tabs value={dateRange} onValueChange={setDateRange} className="shrink-0">
            <TabsList className="bg-slate-100 h-9 p-1 rounded-xl">
              <TabsTrigger value="7d" className="text-[11px] h-7 rounded-lg data-[state=active]:bg-white data-[state=active]:text-orange-700 data-[state=active]:shadow-sm">{tFilter('7days')}</TabsTrigger>
              <TabsTrigger value="30d" className="text-[11px] h-7 rounded-lg data-[state=active]:bg-white data-[state=active]:text-orange-700 data-[state=active]:shadow-sm">{tFilter('30days')}</TabsTrigger>
              <TabsTrigger value="all" className="text-[11px] h-7 rounded-lg data-[state=active]:bg-white data-[state=active]:text-orange-700 data-[state=active]:shadow-sm">{tFilter('all')}</TabsTrigger>
            </TabsList>
          </Tabs>
          ...
          <Tabs value={timeOfDay} onValueChange={setTimeOfDay} className="shrink-0">
            <TabsList className="bg-slate-100 h-9 p-1 rounded-xl">
              <TabsTrigger value="24h" className="text-[11px] h-7 rounded-lg uppercase data-[state=active]:bg-white data-[state=active]:text-orange-700 data-[state=active]:shadow-sm">{tFilter('24h')}</TabsTrigger>
              <TabsTrigger value="am" className="text-[11px] h-7 rounded-lg uppercase data-[state=active]:bg-white data-[state=active]:text-orange-700 data-[state=active]:shadow-sm">{tFilter('am')}</TabsTrigger>
              <TabsTrigger value="pm" className="text-[11px] h-7 rounded-lg uppercase data-[state=active]:bg-white data-[state=active]:text-orange-700 data-[state=active]:shadow-sm">{tFilter('pm')}</TabsTrigger>
            </TabsList>
          </Tabs>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <Select value={contextFilter} onValueChange={setContextFilter}>
              <SelectTrigger className="h-9 min-w-[100px] text-xs font-bold border-slate-200 bg-slate-50 hover:bg-slate-100 focus:ring-orange-100">
          ...
          <ChartContainer config={chartConfig} className="w-full h-[400px]">
            <LineChart data={finalData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
              ...
              <ChartTooltip
                ...
                content={
                  <ChartTooltipContent
                    // Forcem indicator="dot" per evitar que el component amagui el label
                    indicator="dot"
                    className="w-[200px] rounded-2xl border-none shadow-2xl bg-white/95 backdrop-blur-md p-3"
                    labelFormatter={(value, payload) => {
                      // En Recharts, si el valor no ve pel labelFormatter directament, 
                      // el busquem dins del payload del primer item
